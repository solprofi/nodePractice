...
handlers.index = (data, callback) => {
  if (data.method === 'get') {
    const templateData = {
      ...
      'head.description': 'The DESCRiption for the app. A lot of text here.',
      'body.class': 'index',
    };

    helpers.getTemplate('index', templateData, (err, template) => {
      if (!err && template) {
        helpers.addUniversalTemplates(template, templateData, (err, str) => {
          if (!err && str) {
...
handlers.deletedSession = (data, callback) => {
  if (data.method === 'get') {
    const templateData = {
...
handlers.createSession = (data, callback) => {
  if (data.method === 'get') {
    const templateData = {
      ...
      'head.description': 'Please enter the phone number and password',
...
handlers.createAccount = (data, callback) => {
  if (data.method === 'get') {
    const templateData = {
      ...
      'head.description': 'Sign up is easi boi.',
...
handlers.public = (data, callback) => {
  if (data.method === 'get') {
    // get the requsted filename
    const trimmedName = data.trimmedPath.replace('public/', '').trim();

    if (trimmedName.length > 0) {
      helpers.getStaticAsset(trimmedName, (err, asset) => {
        if (!err && asset) {
          // determine the content type
          let contentType = 'plain';

          if (trimmedName.includes('css')) {
            contentType = 'css';
          } else if (trimmedName.includes('jpg')) {
            contentType = 'jpg';
          } else if (trimmedName.includes('png')) {
            contentType = 'png';
          } else if (trimmedName.includes('ico')) {
            contentType = 'ico';
          }
...
handlers._tokens.post = (data, callback) => {
  // check for all required fields
  _performance.mark('started the function');
  let {
    phone,
    password,
  } = data.payload;
  password = helpers.checkName(password);
  phone = helpers.checkPhone(phone);
  _performance.mark('validated data');

  if (phone && password) {
    // check if the user exists
    _data.read('users', phone, (err, userData) => {
      _performance.mark('looked up the user');

      if (!err && userData) {
        // hash the password and compared it to the stored password
        const hashedPassword = helpers.hash(password);
        _performance.mark('hashed the password');
        ...
        if (hashedPassword === userData.hashedPassword) {
          ...
          if (id) {
            ...
            _data.create('tokens', id, tokenObject, (err) => {
              _performance.mark('created the token');

              _performance.measure('start to end', 'started the function', 'created the token');
              _performance.measure('looking up the user', 'validated data', 'looked up the user');
              _performance.measure('hashing the password', 'looked up the user', 'hashed the password');

              const measurments = _performance.getEntriesByType('measure');

              measurments.forEach((measurment) => {
                debug('\x1b[36m%s\x1b[0m', `${measurment.name} ${measurment.duration}`);
              });
        ...
        _data.read('users', phone, (err, userData) => {
          if (!err && userData) {
            ...
            if (checks.length < config.maxChecks) {
              ...
              const hostName = helpers.checkString(parsedUrl.hostname);
...
handlers.exampleError = (data, callback) => {
  throw new Error('This is an expample error');
};
