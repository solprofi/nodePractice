...
workers.processCheckOutcome = (originalCheckData, checkOutcome) => {
  ...
  _data.update('checks', newCheckData.id, newCheckData, (err) => {
    if (!err) {
      ...
      console.log('Error saving the updated check');
    }
  });
};
...
workers.alertUser = (checkData) => {
  ...
  helpers.sendTwilioSms(checkData.phone, message, (err) => {
    if (!err) {
      console.log('Succcess: user was alerted to a status change via sms');
      console.log(message);
    } else {
      console.log('Error. Could not send the sms', err);
    }
  });
};
      ...
      logs.forEach((logName) => {
        ...
        _logs.compress(logName, newFileId, (err) => {
          if (!err) {
            ...
            console.log('Error compressing one of log files');
          }
        });
      });
    } else {
      console.log('Could not find any files to compress');
    }
  });
};
